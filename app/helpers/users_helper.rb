    ...
    user.posts.each do |post|
      posts_html += content_tag(:div, class: 'post') do
        ...
        text_div = content_tag(:div, class: 'text') do
          comment_counter = content_tag(:p, "Comments: #{post.CommentsCounter.nil? ? 0 : post.CommentsCounter}",
                                        class: 'comment-text')
          likes_counter = content_tag(:p, "Likes: #{post.LikesCounter.nil? ? 0 : post.LikesCounter}",
                                      class: 'likes-text')
